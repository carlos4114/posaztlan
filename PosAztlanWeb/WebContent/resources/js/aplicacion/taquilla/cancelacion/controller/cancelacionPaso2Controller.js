...
var cancelacionBoletoControllerPaso2 = angular.module('indexModule').controller('cancelacionBoletoControllerPaso2', function($controller,$scope,taquillaService,ModalService,calculosFactory,cancelacionBoletoService,PropertiesFactory){

	$controller('cancelacionBoletoControllerPaso3',{$scope : $scope });
	...
	$scope.procesarCancelacion = function(cancelacion) {
		if ( $scope.validaForm($scope.formCancelacion))
	...
	$scope.consultarMotivosCancelaciones = function() {
		cancelacionBoletoService.consultarMotivosCancelaciones().success(function(data) {
