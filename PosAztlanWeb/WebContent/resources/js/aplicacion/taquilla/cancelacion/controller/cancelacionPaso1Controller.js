'use strict';
var cancelacionBoletoController = angular.module('indexModule').controller('cancelacionBoletoController', function($controller,$scope,statusFactory,PropertiesFactory,cancelacionBoletoService,ModalService){

	$scope.status = { paso1	:"selected", paso2 :"", paso3	:"", paso4	:"",  paso5  :"", numeroPaso:1	}
	$scope.ticket = null;
	$controller('cancelacionBoletoControllerPaso2',{$scope : $scope });
	...
	$scope.consultarTicket = function(idTicket ) {
		...
		cancelacionBoletoService.consultarTicket(idTicket).success(function(data) {
			$scope.ticket = data;
