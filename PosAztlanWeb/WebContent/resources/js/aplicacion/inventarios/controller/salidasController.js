...
angular.module('indexModule').controller("SalidasController",['$scope','GlobalFactory', 'SalidasService', 'ModalService','DataUtils','ErrorFactory', 
    function($scope,GlobalFactory, SalidasService,ModalService,DataUtils,ErrorFactory){	 			
	...
	$scope.guardar = function(salidaVO) {
		 ...
		 SalidasService.guardar(salidaVO)
		 .then(
	      function(d) {
	    	  if(d.errorCode){
	 ...
	 $scope.consultaEmpresas = function() {
		 SalidasService.consultaEmpresas()
		 .then(
	      function(d) {
	 ...
	 $scope.consultaCanales = function(idEmpresa) {
		 SalidasService.consultaCanales(idEmpresa)
		 .then(
	      function(d) {
	 ...
	 $scope.consultaAlmacenes = function(idCanal) {
		 SalidasService.consultaAlmacenes(idCanal)
		 .then(
	      function(d) {
	 ...
	 $scope.consultaTiposMovInv = function() {
		 SalidasService.consultaTiposMovimientoInv()
		 .then(
	      function(d) {
        	  $scope.listaTiposMovInv = d;
	      },
          function(errResponse){
          });
     }
	 ...
	 $scope.buscar = function(){
		 ...
		 SalidasService.buscar($scope.filtrosVO)
		 .then(
		  function(d) {
	 ...
	 $scope.inicializarValores = function(){
		 ...
		 $scope.listaEmpresas = null;
	 	 ...
	 	 $scope.salidaVO = {idEmpresa: idEmpresa, idCanal: idCanal, idAlmacen: idAlmacen, idTipoMovimiento: null, claveTipoMovimiento:null, productos: []}
	     $scope.errorGeneral='';
		 $scope.mensajeGeneral='';
	 }	 
	 ...
	 $scope.limpiarFormulario = function(){  
		 ...
		 $scope.listaEmpresas = null;
	 	 ...
	 	 $scope.productoVO = {idAlmacen: idAlmacen, idProducto:null, nombre:'', descripcion:'', 
	    		 ...
	    		 idMarca:null, nombreMarca:null, precioUnitario:null, nacional:null, cantidad:null,
	    		 existencia: null, seleccionado:null };
	 	 $scope.salidaVO = {idEmpresa: null, idCanal: null, idAlmacen: null, idTipoMovimiento:null, claveTipoMovimiento:null, productos: []};
	 }
